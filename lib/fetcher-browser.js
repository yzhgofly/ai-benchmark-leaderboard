...
(function (global) {
  ...
  function fetchWithProxies(url, timeout) {
    var proxies = [null].concat(CORS_PROXIES);
    var idx = 0;
    function tryNext() {
      if (idx >= proxies.length) return Promise.reject(new Error('all proxies failed'));
      var fn = proxies[idx++];
      var target = fn ? fn(url) : url;
      return httpGet(target, timeout).catch(function () {
        return tryNext();
      });
    }
    return tryNext();
  ...
  function fetchArtificialAnalysis() {
    return fetchWithProxies('https://artificialanalysis.ai/leaderboards/models', 30000).then(function (text) {
  ...
  function fetchLivebench() {
    return fetchWithProxies('https://livebench.ai/', 25000).then(function (text) {
      var today = todayIso();
  ...
  function fetchVellumSet() {
    return fetchWithProxies('https://www.vellum.ai/llm-leaderboard', 30000).then(function (text) {
      var today = todayIso();
