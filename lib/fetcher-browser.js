...
(function (global) {
  ...
  function fetchArtificialAnalysis() {
    // 通过 allorigins.win CORS 代理访问
    return httpGet('https://api.allorigins.win/raw?url=' + encodeURIComponent('https://artificialanalysis.ai/leaderboards/models'), 30000).then(function (text) {
  ...
  function fetchLivebench() {
    // 通过 allorigins.win CORS 代理访问（服务端直连被 CORS 阻止）
    return httpGet('https://api.allorigins.win/raw?url=' + encodeURIComponent('https://livebench.ai/'), 25000).then(function (text) {
      var today = todayIso();
